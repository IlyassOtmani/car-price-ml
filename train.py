...
# Car Price Dataset from Kaggle (hellbuoy/car-price-prediction)
...
# Extract car brand from CarName (split on first space)
...
print(f"Unique car brands: {df['CarBrand'].nunique()}")
...
with open("Results/metrics.txt", "w") as outfile:
    outfile.write(f"Car Price Prediction Model Performance\n")
    outfile.write(f"=" * 50 + "\n")
    ...
    outfile.write(f"- The model explains {r2*100:.2f}% of the variance in car prices\n")
    outfile.write(f"- Training set size: {len(X_train)} samples\n")
    outfile.write(f"- Test set size: {len(X_test)} samples\n")

# Create visualization: Predicted vs Actual
fig, axes = plt.subplots(2, 2, figsize=(15, 12))

# Plot 1: Predicted vs Actual
axes[0, 0].scatter(y_test, predictions, alpha=0.6, s=50, edgecolors='k', linewidth=0.5)
...
axes[0, 0].set_title('Predicted vs Actual Car Prices', fontsize=14, fontweight='bold')
axes[0, 0].grid(True, alpha=0.3)

# Plot 2: Residuals
residuals = y_test - predictions
axes[0, 1].scatter(predictions, residuals, alpha=0.6, s=50, edgecolors='k', linewidth=0.5)
...
# Plot 3: Distribution of predictions vs actual
axes[1, 0].hist(y_test, bins=20, alpha=0.5, label='Actual', color='blue', edgecolor='black')
axes[1, 0].hist(predictions, bins=20, alpha=0.5, label='Predicted', color='red', edgecolor='black')
...
axes[1, 1].axvline(mae, color='r', linestyle='--', lw=2, label=f'MAE: ${mae:,.2f}')
axes[1, 1].legend()
